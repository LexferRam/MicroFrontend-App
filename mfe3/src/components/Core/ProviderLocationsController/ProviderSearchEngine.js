...
const nominatimPredicate =
  "&country=venezuela&format=json&polygon_geojson=1&addressdetails=1"
const wordsToDelete = [
  "Consultorio",
  "Casco",
  "abajo",
...
export default (async function ProviderSearchEngine(props) {
  ...
  // console.log(itemConsult)
  const direction =
    itemConsult.DIRECCION === undefined
      ? itemConsult.DIRECCION_PROVEEDOR
      : itemConsult.DIRECCION

  const NameProvider =
    itemConsult.NOMCLINICA === undefined
      ? itemConsult.NOMBRE_PROVEEDOR
      : itemConsult.NOMCLINICA

  categoryForCompare = props.item[1].split(",")
  // "hospital,clinic,pharmacy,car_repair,workshop"
  searchLevel = props.searchLevel
  if (NameProvider != null && direction != null && searchLevel < 2) {
    provider = ""
    providerArr = []
    if (searchLevel === 0) {
      provider = NameProvider.trim()
    } else if (searchLevel === 1) {
      provider = direction.trim()
    }
    for (var i = 0; i < 8; i++) {
    ...
    provider = provider.replace(/\s+/g, "+")
    provState =
      itemConsult.ESTADO_PROVEEDOR === undefined
        ? itemConsult.DESCESTADO.trim()
        : itemConsult.ESTADO_PROVEEDOR.trim()
    provStateArr = provState.replace(/\s+/g, " ")
    provState = provState.replace(/\s+/g, "+")
    provCity =
      itemConsult.CIUDAD_PROVEEDOR === undefined
        ? ""
        : itemConsult.CIUDAD_PROVEEDOR.trim()
    provCityArr = provCity.replace(/\s+/g, " ")
    provCity = provCity.replace(/\s+/g, "+")
    provCounty =
      itemConsult.MUNICIPIO_PROVEEDOR === undefined
        ? ""
        : itemConsult.MUNICIPIO_PROVEEDOR.trim()
      ...
      if (sw.toUpperCase() === "AV" || sw.toUpperCase() === "AVENIDA") {
        ...
        if (typeof shortWord[swIndex + 1] !== "undefined") {
          if (shortWord[swIndex + 1].length <= 3) {
            tmp =
              sw + "+" + shortWord[swIndex + 1] + "+" + shortWord[swIndex + 2]
          } else {
    ...
    if (provider.length > 0) {
      if (
        provState.toUpperCase() === "DISTRITO+CAPITAL" &&
        provCounty.toUpperCase() !== "LIBERTADOR"
      ) {
        resultRequest =
          nominatimDomain +
          provider +
    ...
    if (resNomin.data.length > 0 && resNomin.status === 200) {
      // Verifica si hubo resultado desde nominatim
      ...
      if (
        selectedServ.length > 0 &&
        resNomin.data.length > 0 &&
        searchLevel === 1
      ) {
        coordsToMap = []
        selectedServ.map(mr => {
      ...
      if (
        ...
        // si el filtro por tipo no entregó nada pero hubo respuesta del api entra
      ...
      if (selectedServ.length > 0 && searchLevel === 0) {
        //Si la busqueda es por nombre y hubo resultado con el tipo del form select de tipo
        coordsToMap = []
        selectedServ.map(mr => {
          coordsToMap.push([mr.lat, mr.lon, mr.display_name])
          return null
      ...
      // Si no hubo resultado desde nominatim cuando busca por string completo de dirección, hace esto
      if (searchLevel === 0) {
        resultSearch = { providerMap: [], statusType: "empty" }
        setTimeout(() => {
          props.coordProvReturned(resultSearch)
        ...
        setTimeout(async () => {
          if (resIter.length === 1) {
          ...
          } else if (resIter.length > 1) {
            // Sin son multiples direcciones, entra y busca primero por categoría preselecionada...
            selectedServ = []
            ...
            if (selectedServ.length === 0) {
              ...
              if (selectedServ.length > 0) {
                ...
                arrWSum.map(ssv => {
                  if (ssv[1] === bigSum) {
                    if (limitAddress < 15) {
                      //Limitar la cantidad de marcadores a mostrar
              ...
              //Si se encontró resultado con el filtro por tipo preseleccionado, se muestra en el mapa
              if (selectedServ.length > 0) {
                coordsToMap = []
                selectedServ.map(mr => {
                  coordsToMap.push([mr.lat, mr.lon, mr.display_name])
            ...
            /*else if (resIter.length > 1) { Si la búsqueda por iteraciones trajo resultados entra.... sino hace lo de abajo */
            resultSearch = { providerMap: [], statusType: "empty" }
            props.coordProvReturned(resultSearch)
          }
        }, timeToCall + 2000)
