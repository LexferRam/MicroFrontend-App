...
const EventDetail = (props) => {
  ...
  async function getEventList() {
    ...
    const response = await Axios.post(
      "/dbo/team_manager/get_schedule_detail",
      params
    )
  ...
  return (
    <Card>
      <CardHeader>
        <h4
          sx={{ ...STYLES_CALENDAR.modalTitle }}
        >{`${event.title} - ${event.date}`}</h4>
      </CardHeader>
      <CardBody>
        <TableMaterial
          options={{
      ...
      {showModal && (
        <DetailUsersAssignment
          taskId={taskId}
          handleCloseModalUsers={handleCloseModalUsers}
        />
      )}
    </Card>
  )
