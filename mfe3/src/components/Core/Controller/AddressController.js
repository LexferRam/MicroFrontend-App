...
export default function AddressController(props) {
  ...
  async function getMunicipalities() {
    ...
    const result = await Axios.post(
      "/dbo/toolkit/get_list_of_municipalities",
      params
    )
  ...
  async function getUrbanizations() {
    ...
    const result = await Axios.post(
      "/dbo/toolkit/get_list_of_postal_code",
      params
    )
  ...
  function getPostalCode() {
    if (urbanizations) {
      const postal = urbanizations.find(
        (element) => element.CODIGO === urbanization
      )
      setPostalCode(postal.ZONAPOSTAL)
      setValue(`p_postal_code_${index}`, postal.ZONAPOSTAL)
    }
  }
  ...
  useEffect(() => {
    if (municipality) {
      ...
      if (!initial) {
        ...
        setValue(`p_urbanization_id_${index}`, "")
      }
    } else {
  ...
  useEffect(() => {
    if (city) {
      ...
      if (!initial) {
        ...
        setValue(`p_municipality_id_${index}`, "")
      }
    } else {
