...
export default function PlanColumnPDF(props){
  ...
  const {plans} = objPDF;
  const [markSumModified,setMarkSumModified] = useState(false);
  const [markRateModified,setMarkRateModified] = useState(false);
  ...
  useEffect(() => {
    setMarkSumModified(plan.sum_modified_esp && plan.sum_modified_esp === 'S'?' (*) ':' ')
    setMarkRateModified(plan.rate_modified_esp && plan.rate_modified_esp === 'S'?' (**) ':' ') 
  }, []);
